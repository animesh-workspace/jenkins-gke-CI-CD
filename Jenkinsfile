pipeline {
    ...
    stages {
        stage('checkout') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/main' ]], extensions: [], userRemoteConfigs: [[url: 'https://github.com/animesh-workspace/jenkins-gke-CI-CD.git', credentialsId: 'github']]]) 
            }
        }
        stage('maven build') {
            steps {
